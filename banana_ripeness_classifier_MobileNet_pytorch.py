...
# -------------------------------
class BananaRipenessMobileNet(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(BananaRipenessMobileNet, self).__init__()
...
if __name__ == '__main__':
    ...
    model = BananaRipenessMobileNet(num_classes=num_classes).to(device)
    ...
    early_stopping_head = EarlyStopping(patience=5, min_delta=0.1)
    ...
    early_stopping_fine = EarlyStopping(patience=5, min_delta=0.1)
    ...
    model_path = os.path.join(model_dir, "banana_ripeness_mobilenet_pytorch.pth")
    ...
    plt.axvline(x=len(history_head['train_acc'])-1, color='red', linestyle='--')
    ...
    plt.axvline(x=len(history_head['train_loss'])-1, color='red', linestyle='--')
    plt.title('MobileNet Loss')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, "training_history_mobilenet.png"), dpi=300)
    plt.close()
    ...
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, "test_confusion_matrix_mobilenet.png"), dpi=300)
    plt.close()
