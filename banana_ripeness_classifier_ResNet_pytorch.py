...
# -------------------------------
class BananaRipenessResNet(nn.Module):
    def __init__(self, num_classes, pretrained=True):
        super(BananaRipenessResNet, self).__init__()
...
if __name__ == '__main__':
    ...
    model = BananaRipenessResNet(num_classes=num_classes).to(device)
    ...
    model_path = os.path.join(model_dir, "banana_ripeness_resnet_pytorch.pth")
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, "training_history_resnet.png"), dpi=300)
    plt.close()
    ...
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, "test_confusion_matrix_resnet.png"), dpi=300)
    plt.close()
